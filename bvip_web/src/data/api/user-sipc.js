...
const url = (
  window.location.hostname === '47.111.138.2'
  || window.location.hostname === 'localhost'
  || window.location.hostname === '127.0.0.1'
) ? 'http://47.111.138.2:8889' : window.location.origin
...
export default class UserDefault {
  ...
  static getUser() {
    ...
    return Http.get('/user').then(user => ({
      ...
      if (status === 400 || status === 401) AuthToken.setToken('')
      throw e
    })
  }
   ...
   */
  static sendSms({ username, use_type = 0, id, img_code = '', country_id = '' }) {
    return Http.postForm('/SMS', { username, use_type, id, img_code, country_id })
  }
  ...
  static withdraw({ asset, address_id, amount, password, sms_code = '', email_code = '', two_step_code = '' }) {
  ...
  static getRebaseRecords({ offset = 0, limit = 10 }) {
    return Http.get(
      `${url}/api/activity/invite_commission_activity/recommend/commission/log`,
      { offset, limit },
    ).then(res => ({
      ...res,
      // recordsC: res.records && res.records.map(inviteC),
