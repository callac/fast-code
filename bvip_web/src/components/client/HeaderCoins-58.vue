    ...
    <section v-for="(h, i) in myHeads" class="item" :key="i">
      ...
      <em class="value-wrap">
        ...
        <template v-if="i===0">
          ...
          <div class="coins">
            <div v-for="(c, j) in coins" class="zone" :key="j">
              {{c.name}}
              <span class="iconfont icon-arrowdropdown-copy"></span>
              <div class="table-wrap">
                <my-table class="coins-table"
                          type="extend"
                          :heads="tableHeads" :data="tableData[j]"
                          :headTrStyle="{height: '.36rem'}"
                          :trStyle="{height: '.34rem'}"
                          :scrollbarProps="{isMobile, maxHeight:'50vh'}"
                          @select="select(tableData[j][$event].ID)"
                          @clickTr="chose(j, $event)"
                          @sort="sortType=$event"/>
              </div>
            </div>
          </div>
...
// 模板 58 的币币交易页面显示在 header 的交易对列表信息
export default {
  name: 'HeaderCoins',
  mixins: [symbolsMixin],
  beforeMount() {
      ...
      return [
        ...
        { name: 'coin', alias: 'symbol1', style: { padding: 0, textAlign: 'left' } },
        {
          ...
          formatter: (item, alias) => {
            ...
            return `${price}<span style="margin: 0 0 0 .06rem;font-size: .14rem; color: rgba(255,255,255,.5)}">${this.$cEx(item[`${alias}1`])}</span>`
          },
        },
        {
          name: 'change',
          ...
          formatter: (item) => {
            ...
            return color ? `<span class="${color}">${value}</span>` : `<span style="color: rgba(255,255,255,.5)">${value}</span>`
          },
        },
