<template>
  <footer class="content client-footer">
    <div class="columns">
      ...
      return {
        ...
        items: orderBy([
          ...(aboutUs.some(f => f.label_name === '关于我们')
            ? [] : [{ name: 'about-us', route: '/article/about-us', order: 1 }]),
          { name: 'about-us.privacy-policy', route: '/article/privacy-policy', order: 2 },
          { name: 'about-us.login-agreement', route: '/article/user-agreement', order: 3 },
    ...
    help() {
      return {
        ...
        items: this.footers[Status.footerItems[2].alias].map(item => ({
          ...
          ...(item.api_url ? { href: item.api_url } : { route: `/article/help?classId=${item.id}` }),
          needTran: false,
        })),
      }
    },
