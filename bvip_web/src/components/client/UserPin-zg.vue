<template>
  <div v-if="!user.id" class="user-pin login">
    <router-link class="btn btn-tran transition" :to="'/sign-in?redirectTo='+$route.path">
      {{$t('sign-in')}}
    </router-link>
    <router-link class="btn btn-tran transition" :to="'/sign-up?redirectTo='+$route.path">
      {{$t('sign-up')}}
    </router-link>
  </div>
      ...
      return [
        {
          ...
          children: [
            ...
            { name: 'exchange.manage', route: '/user-center/exchange?type=open' },
            { name: 'user.security', route: '/user-center/security' },
            { name: 'user.invite', route: '/user-center/invite' },
            {
              name: 'user.logout',
              click: this.logout,
            },
