<template>
  <page-container class="mobile-sign-up" title="注册">
    <header>注册</header>
    <div class="wrap">
      <div class="tabs">
        <router-link v-for="(t,i) in types" :key="i"
                     :to="$route.path+'?type=' + t.value
                          + (recommend_code?'&recommend_code=' + recommend_code:'')"
                     :class="{selected: type===t.value}" replace>{{t.name}}
        </router-link>
      </div>
      <div class="forms">
        <mobile-input v-for="item in showItems" v-model="item.value" type="1"
...
export default {
  ...
  methods: {
    ...
    submit() {
      return (this.userAgreement.value ? Promise.resolve() : Promise.reject(this.$t('sign-up.agreement.tip')))
  ...
  components: { MobileInput, Agreement, Privacy },
}
</script>
