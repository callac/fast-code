<template>
  <page-container class="user-content cash-pay-set" :title="$t('pay.set')">
    <tab-head v-model="tab.value" :options="tab.options"/>
    <!--<button v-if="bankCards.length&&add!==true" class="btn btn-main" @click="add=true">-->
    <!--{{$t('add')}}-->
    <!--</button>-->
    <!--<div v-if="bankCards.length&&add!==true" class="cards">-->
    <!--<div v-for="(c, i) in bankCards" class="card" :key="i">-->
    <!--<div class="wrap">-->
    <!--<h3>{{c.bank}}</h3>-->
    <!--<p>{{c.card_number}}</p>-->
    <!--</div>-->
    <!--<div class="btn-group">-->
    <!--&lt;!&ndash;<button class="btn" @click="add=true">{{$t('add')}}</button>&ndash;&gt;-->
    <!--<button class="btn" @click="del(c.ID)">{{$t('del')}}</button>-->
    <!--</div>-->
    <!--</div>-->
    <!--</div>-->
    <p class="tip">{{tip}}</p>
    <template v-if="add!==true">
      <button class="btn btn-add btn-green-fill" @click="add=true">
        {{$t('add')}}
      </button>
    ...
    <div v-else class="forms">
      ...
      <my-input v-for="(item,i) in showVerifyItems" v-model="item.value" type="1" :key="i"
                :item="item"/>
      <div class="btn-group">
        <btn-once class="btn btn-green-fill btn-sure" :clickFn="submit" :useLoading="true">
          {{$t('sure')}}
        </btn-once>
...
export default {
  name: 'CashPaySet',
  components: { TabHead, MyFileInput, MyInput },
  data() {
    return {
      tab: {
        value: '',
