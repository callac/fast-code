...
export default {
  ...
  data() {
    return {
      ...
      heads: {
        base: [
          { name: 'date', alias: 'date', style: { width: '21%', textAlign: 'left' } },
          { name: 'pair', alias: 'symbol1', style: { width: '11.5%', textAlign: 'left' } },
          { name: 'type', alias: 'type', style: { width: '7%', textAlign: 'left' } },
          { name: 'price', alias: 'price', style: { width: '11.6%', textAlign: 'left' } },
          { name: 'amount', alias: 'amount', style: { width: '11.6%', textAlign: 'left' } },
          { name: 'total', alias: 'total', style: { width: '11.6%', textAlign: 'left' } },
          { name: 'executed', alias: 'deal_stock', style: { width: '8.8%', textAlign: 'left' } },
        ],
        open: [
          { name: 'unexecuted', alias: 'left', style: { width: '8.8%', textAlign: 'left' } },
          { name: 'action', style: { width: '7%', textAlign: 'right' } },
        ],
        history: [
          { name: 'average', alias: 'average', style: { width: '8%', textAlign: 'left' } },
          {
            name: 'status',
            style: { width: '9%', textAlign: 'right', whiteSpace: 'nowrap' },
          },
        ],
        details: [
          {
            name: 'commission',
            alias: 'fee',
            style: { width: '11%', textAlign: 'right', padding: '0 .2rem 0 .1rem' },
          },
        ],
      },
      overlayTableStyle: {
  ...
  computed: {
    ...
    detailsHeads() {
      return feeAsset => ([
        {
          ...this.heads.base[0],
          style: { ...this.heads.base[0].style, padding: '0 .1rem 0 .2rem' },
        },
  ...
  watch: {
    symbols: {
      handler(val) {
        if (!this.filters.symbol && val.length > 0) this.filters.symbol = val[0].value
      },
      immediate: true,
    },
    'filters.symbol'(val) {
