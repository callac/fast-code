<template>
  <page-container class="container-inner home">
    ...
    <img :src="require(
            'assets/client/home/radian-img-<<template>>-<<theme>>.png')" alt="" class="radian-img">
    ...
    <div v-if="symbolsArrC" class="content market">
      ...
      <div v-if="mainPrices.length>2" class="main-prices">
        <router-link v-for="(p, i) in mainPrices" class="main-price" :key="i"
                     :to="'/exchange?coin='+p.symbol">
          <h2><span><span class="coin-flag">{{p.base_asset}}</span>/{{p.quote_asset}}</span>
            <em class="rate" :class="getColor(p.rate)">{{(p.rate||0).toFixed(2)+'%'}}</em>
          </h2>
          <em class="price" :class="getColor(p.rate)">
            {{p.last||0}} <span class="unit">{{p.quote_asset}}</span></em>
      ...
      <div class="prices">
        ...
        <tab v-model="tab.value" :options="tabOptions"
             :tabStyle="{ padding: '.05rem .2rem'}"
             :selectedStyle="{color: '#fff'}"/>
        <!-- <div class="market-search">
          <img :src="require(
            'assets/client/home/market-search-icon-<<template>>-<<theme>>.png')" alt="">
        </div> -->
        <my-table class="table"
                  :heads="heads"
                  :data="tableData"
                  :headTrStyle="{height:'.44rem',opacity:'.6'}"
    ...
    <section class="content apps">
      ...
      <img class="app-img" :src="require('assets/client/home/home-apps-jp2-jp2.png')" alt="">
    </section>
  </page-container>
</template>
...
import Announce from 'components/client/Announce-jp2'
import Banner from 'components/client/Banner-jp2'
import Tab from 'components/client/Tab-jp2'
...
export default {
  name: 'Home',
  components: { Tab, Banner, Announce },
  mixins: [symbolsMixin],
  data() {
    return {
      wrap: '',
        ...
        items: [
          {
            ...
            icon: require('assets/client/home/feature-safe-icon-jp2-jp2.png'),
          },
          {
            name: 'advantages.2.title',
            desc: 'advantages.2.desc',
            icon: require('assets/client/home/feature-withdraw-icon-jp2-jp2.png'),
          },
          {
            name: 'advantages.3.title',
            desc: 'advantages.3.desc',
            icon: require('assets/client/home/feature-engine-icon-jp2-jp2.png'),
          },
          {
            name: 'advantages.4.title',
            desc: 'advantages.4.desc',
            icon: require('assets/client/home/feature-profession-icon-jp2-jp2.png'),
          },
        ],
      },
      article: {
