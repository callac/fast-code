...
> 建议2: 页面可能共用的语句写在 `common-default.js` 中，其它的词汇语句可按照业务功能做命名空间写在 `index.js`，或者使用单独的文件
...
# serve with hot reload at localhost:8080, please set the process.env.theme before run `npm start`, you can use module `cross-env`(global installed), and process.env.theme default to `default`
cross-env template=[template] theme=[the theme] npm start
...
# templates/themes defaults to '', see how it works in `./build/webpack.prod.conf.js`
# HappyPack for build will be forbidden when NO_HAPPYPACK set to non-null
cross-env templates=[templates] themes=[themes] NO_HAPPYPACK=[value] npm run build

# build for production and view the bundle analyzer report
npm run build --report

# run server in production env for main-sub station module
cross-env port=[port] expireTime=[expireTime] defaultTemplate=[template] defaultTheme=[theme] npm run server
```

For a detailed explanation on how things work, check out the [guide](http://vuejs-templates.github.io/webpack/) and [docs for vue-loader](http://vuejs.github.io/vue-loader).

## 代码提交(<span style="color:red">重要</span>)
使用了 `commitizen` 规范 commit message, 所以：
<span style="color:red">使用 `npm run commit` 代替 `git commit`</span>
