...
module.exports = (product) => {
  const { template, theme, mask } = product
  const productLoader = product.loader
  const webpackConfig = merge(baseWebpackConfig(mask, template, theme, productLoader), {
    entry: {
      app: ['./src/main.js'],
    },
    output: {
    ...
    externals: {
      '$router/ThemeComponents': 'ThemeComponents'
    },
    module: {
      rules: [
        happypackConf.rules.vueCommon(productLoader),
      ],
    },
    ...
    plugins: [
      ...
      new HtmlWebpackPlugin({
        filename: config.build.index(`index-${mask}.html`),
      ...
      new CopyWebpackPlugin([
        {
          from: utils.pathResolve('/static/chart_main'),
          to: `chart_main`,
          ignore: ['.*'],
        },
        {
      ...
      ...happypackConf.plugins.vueCommon,
      VueReference,
      UIAndUtils,
    ],
  })
