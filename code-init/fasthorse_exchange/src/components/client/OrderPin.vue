  ...
  <transition name="fade-enter">
    <section class="order-pin">
      ...
      <span v-for="(h,i) in heads" :key="i">
        {{$t(h.name)}}:{{h.name==='time'?dateFormatter(order[h.alias]):order[h.alias]}}
      </span>
      </h2>
      <div class="wrap">
        <h3 :class="type.name==='sell-1'?'font-red':'font-green'">
          {{$t(type.name)}}{{order.coin}}</h3>
        <div class="money">
          <span class="total"><span
            class="unit">{{totalAmount.unit}}</span>{{totalAmount.amount}}</span>
          <span v-for="(h,i) in heads1" :key="i">
          <span class="name">{{$t(h.name)}}:</span> {{order[h.alias]}}</span>
        </div>
        <div class="pay-type">
          <img :src="payType.icon" alt="">
          <span>{{$t(payType.name)}}{{$t('transfer')}}</span>
        </div>
        <div class="payee">
          <template v-for="(h,i) in heads2">
            <div v-if="order[h.alias]" class="item" :key="i">
              ...
              <button v-if="h.copy" class="btn-green"
                      @click="copy(order[h.alias])">{{$t('copy')}}
              </button>
            </div>
          </template>
        </div>
        <div class="status-wrap">
          ...
          <template v-if="status.name==='pay.wait'">
            <div class="btn-group">
              <button class="btn-status btn-red-fill" @click="cancel">
                {{$t('cancel')}}
              </button>
              <button v-if="type.name==='buy-1'" class="btn-status btn-green-fill" @click="paid">
                {{$t('paid')}}
              </button>
            </div>
...
export default {
  ...
  data() {
    return {
      heads: [
        { name: 'order.num', alias: 'order_no' },
        { name: 'time', alias: 'UpdatedAt' },
      ],
      heads1: [
