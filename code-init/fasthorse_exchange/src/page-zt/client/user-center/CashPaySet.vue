<template>
  <page-container class="user-content cash-pay-set" :title="$t('pay.set')">
    ...
    <div class="cash-pay-set-detail">
      <div class="cash-pay-set-title">{{$t('pay.set')}}</div>
      <p class="tip">{{tip}}</p>
      <template v-if="add!==true">
        <button class="btn btn-add btn-blue" @click="add=true">
          {{$t('add')}}
        </button>
      ...
      <div v-else class="forms">
        ...
        <my-input v-for="(item,i) in showVerifyItems" v-model="item.value" type="1" :key="i"
                  :item="item"/>
        <div class="btn-group">
          <btn-once class="btn btn-blue btn-sure" :clickFn="submit" :useLoading="true">
            {{$t('sure')}}
          </btn-once>
...
// const OSS = import('utils/Oss' /* webpackChunkName:'OSS' */)

export default {
  name: 'CashPaySet',
  components: { NavHead, MyFileInput, MyInput },
  data() {
    return {
      nav: {
        value: 1,
