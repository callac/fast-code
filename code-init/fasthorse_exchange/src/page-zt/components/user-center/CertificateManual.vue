    ...
    <template v-for="(item, i) in picItems">
      ...
      <div class="input-content" :key="'div-'+i">
        ...
        <div class="example">
          ...
          <p class="ex">{{$t('certificate.example')}}</p>
        </div>
      </div>
    </template>
...
export default {
  ...
  components: { TabHead, MyFileInput },
  data() {
    return {
      example: {
  ...
  methods: {
    ...
    uploadImg(img_type, files) {
      ...
      OSS.then(({ ossUpload }) => ossUpload(this.oss, files[0])
