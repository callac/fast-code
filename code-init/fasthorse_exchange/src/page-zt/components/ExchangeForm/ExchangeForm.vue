  ...
  <form class="exchange-form" @submit.prevent="">
    <div class="balance">
      <template v-if="user.id">
        {{balance}} {{$t('balance')}}:
        <span class="value">{{balanceVal}}</span>
      </template>
    </div>
    <my-input v-for="(item,i) in items" v-model="item.value" :key="i" :item="item"/>
    <div class="ranges">
      <button v-for="(r,i) in ranges" class="range" :key="i" type="button"
